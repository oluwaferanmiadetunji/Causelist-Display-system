            ...
            <div class="container text-center">
				...
				<table class="table table-striped table-dark table-bordered">
                    ...
                    <?php
                        ...
                        $total_pages_sql = "SELECT COUNT(*) FROM cases";
                        $result = mysqli_query($conn,$total_pages_sql);
                        $total_rows = mysqli_fetch_array($result)[0];
                        $total_pages = ceil($total_rows / $no_of_records_per_page);

                        $sql = "SELECT * FROM cases ORDER BY date DESC LIMIT $offset, $no_of_records_per_page";
                        $res_data = mysqli_query($conn,$sql);
                        while($row = mysqli_fetch_array($res_data)){
                    ?>
                            <tbody>
                                <tr>
                                    <th scope="row"><?php echo $row['suit_number']; ?></th>
                                    <td><?php echo $row['parties']; ?></td>
                                    <td><?php echo $row['court_room']; ?></td>
                                    <td><?php echo $row['date']; ?></td>
                                    <td style="width:15px;">
                                        <a class="warning btn btn-secondary btn-outline-warning btn-sm" href="edit_case.php?edit=<?php echo $row['id']; ?>">Edit</a>
                                    </td>
                                    <td style="width:15px;">
                                        <a class="danger btn btn-secondary btn-outline-danger btn-sm" href="includes/delete.php?id=<?php echo $row['id']; ?>">Delete</a>
                                    </td>
                                </tr>
                            </tbody>
                        <?php
